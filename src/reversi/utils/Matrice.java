...
public class Matrice {
	...
	public void chercherMouvements(int typePiece) {
		...
		for (int i = 0; i < pieces; i++) {
			...
			if (get(i) != Piece.VIDE || coupsPossibles.contains(i))
				continue;
			...
			for (int dH = HAUT; dH <= BAS; dH++) {
				for (int dV = GAUCHE; dV <= DROITE; dV++) {
					...
					for (int j = 1; j < pieces; j++) {
						...
						if (j == 1 && (typeVoisine == Piece.VIDE || typeVoisine == typePiece))
							break;
						
						if (ok && typeVoisine == Piece.VIDE)
							break;
						...
						if (typeVoisine == typePiece) {
							if (ok)
							{
								System.out.println("Valide en " + Coordonnees.toString(x, y));
						...
						if (typeVoisine != Piece.VIDE && typeVoisine != typePiece)
							ok = true;
